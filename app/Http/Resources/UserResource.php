...
class UserResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $arrayReturn =[
            ...
            'type' => User::getUserType($this->user()->type),
            'verification' => $this->user()->verification,
            'mobile' => $this->user()->mobile,
            'gender' => User::getGenderType($this->user()->gender),
            'title' => $this->user()->title,
            'nationality_code' => $this->user()->nationality_code,
            'governorate_id' => Governorate::getNameEN($this->user()->governorate_id),
            'city_id' => City::getNameEN($this->user()->city_id),
        ...
        if ($this->user()->type == '2'){
            ...
            if (isset($doctor->id)) {
                $doctor = [
                    'doctor_id'=> $doctor->id,
                    'specialty'=> Specialty::getNameEN($doctor->specialty),
                    'cv'=> $doctor->cv,
